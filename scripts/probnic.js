$(function() {
	...
	function mouseDirection(e) {
		...
		relMouseX = (mouseX - $(e.target).offset().left);
		relMouseY = (mouseY - $(e.target).offset().top);
	}
	...
	function initButton($button) {
		...
		canvas.height = buttonHeight + 200;
		...
		// Start render
		renderCanvas($canvas, context, pointsA, pointsB);
	}
	...
	function Point(x, y, level) {
		this.x = this.ix = 100 + x;
		this.y = this.iy = 100 + y;
		this.vx = 0;
		this.vy = 0;
		this.cx1 = 0;
		this.cy1 = 0;
	...
	Point.prototype.move = function() {
		...
		if ((mouseDirectionY > 0 && relMouseY > this.y) || (mouseDirectionY < 0 && relMouseY < this.y)) {
			if (relDist > 0 && relDist < 1) {
				this.vy = (mouseSpeedY / 3) * relDist; // 4
			}
		}
		this.vy *= (1 - damping);
		this.y += this.vy;
	 ...
	 * Render canvas
	 */
	function renderCanvas($canvas, context, pointsA, pointsB) {
		// rAF
		requestAnimationFrame(function() { renderCanvas($canvas, context, pointsA, pointsB) });
		...
		var gradient = context.createRadialGradient(gradientX, gradientY, 200 + (200 * distance), gradientX, gradientY, 0);
		gradient.addColorStop(0, '#7ed21a');
		...
		for (var j = 0; j <= 1; j++) {
			...
			for (var i = 0; i < points.length; i++) {
				...
				var val = 30 * 0.552284749831; // 0.552284749831
